      ...
      <van-tab title="用户">
        <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
          <div class="user_list" v-for="(item,index) in user_list" :key="index">
            ...
            <div @click="otherself(item.LinkUserId)" class="user_list_middle">
            ...
            <!-- <i style="position:absolute;top:0.3rem;right:1.8rem;color:#B1B1B1;font-size:23px" class="iconfont icon-xiaoxiduihuaxinxi"></i> -->
            <i
      ...
      <van-tab title="店铺">
        ...
        <van-list v-model="loading1" :finished="finished1" finished-text="没有更多了" @load="onLoad1">
          <div class="shop_list" v-for="(item,index) in shop_list" :key="index">
            <img
              @click="todianpu(item.SellerId,item.SellerType)"
              v-if="item.BrandImg"
              :src="item.BrandImg"
              alt
            />
            <img
              @click="todianpu(item.SellerId,item.SellerType)"
              v-else
              src="../../../assets/images/shoplist_banner.jpg"
              alt
            />
            <div @click="todianpu(item.SellerId,item.SellerType)" class="shop_list_middle">
              <div class="shop_list_middle_name">{{item.SellerName}}</div>
              <div
                class="shop_list_middle_count"
              >{{item.CommodityCount}}个商品 {{item.SellerFansCount}}个粉丝</div>
            </div>
...
export default {
  ...
  methods: {
    ...
    get_renlist() {
      request
        .post(this.$api + "/mobile/GetMyAttentionPageList", {
          Page: this.page,
          Rows: 9,
          Order: "asc",
          Sort: "Id"
