<template>
  <div class="contain">
    ...
    <header style="border-bottom:1px solid #e6e6e6" class="head">
    ...
    <div class="main">
      ...
      <div class="user">
        <div class="userleft">
          <img @click="otherself" v-if="articleinfo.UserAvatar" :src="articleinfo.UserAvatar" alt />
          <img @click="otherself" v-else src="../../../assets/images/article_touxiang.jpg" alt />
          <span class="lefttop">{{articleinfo.UserName|usernameLength}}</span>
          <span class="leftmiddle">{{articleinfo.ArticleCreateDate}}</span>
        </div>
        <div v-show="isguan==0" @click="guanzhu" class="userright">
          <span>关注</span>
        </div>
        <div v-show="isguan==1" @click="del_guanzhu" class="userright1">
          <span>已关注</span>
        </div>
      </div>
...
export default {
  ...
  mounted() {
    ...
    // this.gettoken();
    console.log("showjvbao" + this.showjvbao);
  ...
  methods: {
    ...
    goback() {
      ...
      if (isAndroid) {
        ...
        console.log("我是要去ios");
        let url =
          this.$domain.url+"articleshua?id=" + this.articleid;
        console.log(url);
    ...
    getdetail() {
      request
        .post(this.$api + "/mobile/GetTopicArticleDetail?id=" + this.articleid)
        .then(res => {
