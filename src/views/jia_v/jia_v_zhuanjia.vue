<template>
  <div>
    ...
    <div class="card_one">
      ...
      <p class="p2">适用于具有美肤相关职业资格证书或附加身份的用户。如具有护肤...</p>
    </div>
    <div class="hang">
      <img src="../../assets/images/jia_v_id.png" alt />
      <span style="margin-left:10px">身份认证</span>
      <div class="over" v-if="this.shenfen === ''" @click="$router.push('/realname')">去完成</div>
      <div class="over" v-else-if="this.shenfen === 'UnApprove'">审核未通过</div>
      <div class="isover" v-else-if="this.shenfen === 'UnAudit'">审核中</div>
      <div class="isover" v-else>已完成</div>
    </div>
    <div class="hang">
      <img class="img1" src="../../assets/images/jia_v_phone.png" alt />
      <span style="margin-left:14px">绑定手机</span>
      <div class="over" v-if="!this.haveMObile.length" @click="$router.push('/set_mobile')">去完成</div>
      <div class="isover" v-else>已完成</div>
    </div>

    <div class="hang">
      <img class="img2" src="../../assets/images/jia_v_up.png" alt />
      <span style="margin-left:12px">提交美肤相关职业资格证书</span>
      <div v-if="!list.length" class="over" @click="fans">去完成</div>
      <div v-else class="isover">已完成</div>
    </div>

    <div class="btn" v-if="isZancun != 'UnAudit'" @click="like">立刻申请成为美妆专家</div>
...
.card_one {
  position: relative;
  width: 670 / @base;
  background: url("../../assets/images/jia_v_zhuanjia.png") no-repeat;
